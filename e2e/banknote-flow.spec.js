...
test.describe('Banknote Flow E2E', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    try {
      ...
      if (!healthResponse.ok()) {
        throw new Error(`PocketBase health check failed: ${healthResponse.status()}`);
      }
    } catch (error) {
      throw new Error(
      ...
      throw new Error(
        `Hermes is not running. Please start it with: npm run dev:`
      );
    }

    // Check Scraper API
    try {
      const scraperHealth = await request.get(`${SCRAPER_URL}/health`, { timeout: 3000 });
      if (!scraperHealth.ok()) {
        throw new Error(`Scraper health check failed: ${scraperHealth.status()}`);
      }
      const healthData = await scraperHealth.json();
      if (healthData.status !== 'ok') {
        throw new Error(`Scraper is not healthy: ${JSON.stringify(healthData)}`);
      }
    } catch (error) {
      throw new Error(
    ...
    try {
      ...
      if (testResponse.status() === 403) {
        ...
        if (errorData.error && (errorData.error.includes('ADMIN_EMAIL') || errorData.error.includes('ADMIN_PASSWORD'))) {
          throw new Error('Scraper is missing admin credentials');
        }
      }
    } catch (error) {
      if (error.message.includes('ADMIN_EMAIL') || error.message.includes('ADMIN_PASSWORD') || error.message.includes('admin credentials')) {
        throw new Error(
  ...
  test.afterAll(async () => {
    ...
    try {
      ...
      let ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || process.env.E2E_ADMIN_PASSWORD;

      if (!ADMIN_EMAIL || !ADMIN_PASSWORD) {
        try {
          const fs = require('fs');
          const path = require('path');
          const envPath = path.join(__dirname, '..', 'backend', 'scraper', '.env');
      ...
      if (!ADMIN_EMAIL || !ADMIN_PASSWORD) {
        console.warn('⚠️  Cannot clean up test data: ADMIN_EMAIL and ADMIN_PASSWORD not set');
        console.warn('   Set E2E_ADMIN_EMAIL and E2E_ADMIN_PASSWORD environment variables for cleanup');
      ...
      if (ADMIN_EMAIL && ADMIN_PASSWORD) {
        try {
          ...
          console.warn('⚠️  Cannot authenticate as admin for cleanup:', error.message);
          adminPb = null;
        }
      }
        ...
        try {
          ...
          for (const user of allE2eUsers) {
            try {
              ...
              const userBanknotes = await adminPb.collection('banknotes').getFullList({
              ...
              const userSubscriptions = await adminPb.collection('subscriptions').getFullList({
                filter: `userId = "${user.id}"`,
              });
              ...
              console.warn(`⚠️  Error cleaning up user ${user.id}:`, error.message);
            }
          }

          if (cleanedCount > 0) {
            console.log(`✅ Cleaned up ${cleanedCount} e2e test user(s) and their associated data`);
          }
        } catch (error) {
  ...
  test('Complete banknote flow: Signup → PMG fetch → Extract data → Save → Verify DB', async ({ page, context }) => {
    ...
    if (!isAlreadySignup) {
      ...
      try {
        ...
        throw new Error('Failed to switch to signup mode. Sign up button/link not found.');
      }
    }
    ...
    page.on('response', async (response) => {
      if (response.url().includes('/api/collections/users') && response.request().method() === 'POST') {
        ...
        if (!response.ok()) {
          try {
            ...
            apiError = { status: response.status(), data: await response.text() };
          }
        }
      }
    });
    ...
    if (apiError) {
      throw new Error(`Signup API error: ${apiError.status} - ${JSON.stringify(apiError.data)}`);
    }

    // If no response at all, PocketBase might not be running
    if (!signupResponse) {
      console.warn('No API response detected - PocketBase might not be running');
      // Check if we can reach PocketBase
      try {
        const healthCheck = await page.request.get(`${POCKETBASE_URL}/api/health`);
        if (!healthCheck.ok()) {
          throw new Error(`PocketBase health check failed at ${POCKETBASE_URL}. Status: ${healthCheck.status()}`);
        }
      } catch (error) {
        if (error.message.includes('ECONNREFUSED') || error.message.includes('fetch failed')) {
          throw new Error(`Cannot connect to PocketBase at ${POCKETBASE_URL}. Please ensure PocketBase is running. The test should start it automatically, but if it fails, run: npm run dev:pocketbase`);
        }
        throw error;
      }
    ...
    try {
      await page.waitForURL(/\/(your-banknotes|community|settings)/, { timeout: 15000 });
    } catch (error) {
      ...
      const notification = page.locator('.mantine-Notification-root, [role="alert"], .mantine-Notification-title').first();
      if (await notification.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        if (await message.isVisible().catch(() => false)) {
          ...
          throw new Error(`Signup failed: ${notificationText} - ${messageText}`);
        }
        throw new Error(`Signup failed: ${notificationText}`);
      }

      // Check for form validation errors
      const validationErrors = await page.locator('.mantine-Input-error, .mantine-InputWrapper-error').all();
      if (validationErrors.length > 0) {
        const errors = await Promise.all(
          validationErrors.map(el => el.textContent())
        );
        throw new Error(`Form validation errors: ${errors.filter(Boolean).join(', ')}`);
      }
    ...
    console.log('[E2E] Auth data from localStorage:', JSON.stringify(authData, null, 2));
    ...
    if (authData && authData.model && authData.model.id) {
      ...
      console.error('[E2E] Could not find user ID in auth data. Full data:', authData);
    ...
    const fetchButton = page.getByRole('button', { name: /fetch.*pmg.*image/i });
    ...
    const responsePromise = page.waitForResponse(
      response => response.url().includes('/api/pmg-images') && response.request().method() === 'POST',
      { timeout: 10000 }
    ).catch(() => null); // Don't fail if no response

    page.on('response', async (response) => {
      if (response.url().includes('/api/pmg-images') && response.request().method() === 'POST') {
        ...
        if (!response.ok()) {
          try {
            ...
            pmgFetchError = { status: response.status(), data: await response.text() };
          }
        }
      }
    });
    ...
    if (pmgFetchError) {
      ...
      if (errorMsg.includes('ADMIN_EMAIL') || errorMsg.includes('ADMIN_PASSWORD')) {
        throw new Error(
      ...
      throw new Error(`PMG fetch failed: ${pmgFetchError.status} - ${errorMsg}. Make sure Hermes is running: npm run dev:hermes`);
    }

    if (!pmgFetchResponse) {
      console.warn('No PMG fetch API response detected. Checking scraper service...');
      // Check if scraper is accessible
      try {
        const healthCheck = await page.request.get(`${SCRAPER_URL}/health`, { timeout: 5000 });
        if (!healthCheck.ok()) {
          throw new Error(`Scraper health check failed with status ${healthCheck.status()}`);
        }
        const healthData = await healthCheck.json();
        if (!healthData.hermes || healthData.hermes !== 'connected') {
          throw new Error(`Hermes is not connected to scraper. Please ensure Hermes is running: npm run dev:hermes`);
        }
        // Scraper is running but PMG fetch didn't respond - this might be a different issue
        console.warn('Scraper is running but PMG fetch API call was not detected. This might indicate a network issue or the fetch is still processing.');
      } catch (error) {
    ...
    try {
      await page.waitForSelector('[class*="LoadingOverlay"], [class*="loading"]', { state: 'hidden', timeout: 30000 });
    } catch {
      ...
      if (response.url().includes('/api/extract-pmg-data')) {
        ...
        if (response.ok()) {
          try {
            ...
            console.log('[E2E] Extraction data:', JSON.stringify(extractionData));
          } catch (e) {
            console.log('[E2E] Could not parse extraction response');
          }
        } else {
          console.error('[E2E] Extraction failed with status:', response.status());
        }
      }
    });
    ...
    try {
      await page.waitForSelector('[class*="LoadingOverlay"]', { state: 'visible', timeout: 5000 });
      await page.waitForSelector('[class*="LoadingOverlay"]', { state: 'hidden', timeout: 120000 }); // Increased to 120s
    } catch {
    ...
    await page.waitForSelector('img[alt*="obverse"], img[alt*="Obverse"]', { timeout: 30000 });
    await page.waitForSelector('img[alt*="reverse"], img[alt*="Reverse"]', { timeout: 30000 });
    ...
    const processingIndicators = await page.locator('[class*="loading"], [class*="spinner"], [aria-busy="true"]').count();
    if (processingIndicators > 0) {
      console.log(`[E2E] Form still processing (${processingIndicators} indicators), waiting...`);
    ...
    for (let i = 0; i < 20; i++) {
      const currentNoteType = await page.evaluate(() => {
        ...
        const usButton = document.querySelector('[data-value="us"][data-active="true"]');
        if (usButton) {
          return 'us';
        }
        const worldButton = document.querySelector('[data-value="world"][data-active="true"]');
        if (worldButton) {
          return 'world';
        }
        return null;
      ...
      console.log(`[E2E] Attempt ${i + 1}: Current note type = ${currentNoteType}`);
    ...
    if (!noteTypeSwitched) {
      console.log('[E2E] Form did not automatically switch to US note mode, will try manually...');
    }
    ...
    // Check pick number
    const pickNumberField = page.locator('input[name="pickNumber"], input[value*="2231"]').first();
    ...
    // Check EPQ checkbox
    const epqCheckbox = page.locator('input[name="isEpq"], input[type="checkbox"][aria-label*="EPQ"]').first();
    ...
    while (!isEnabled && retryCount < maxRetries) {
      console.log(`[E2E] Save button is disabled (attempt ${retryCount + 1}/${maxRetries}), checking form fields...`);
        ...
        if (noteTypeValue !== 'us') {
          ...
          try {
            ...
            const selectors = [
              ...
              page.locator('button:has([alt*="US"]), button:has([title*="US"])'),
            ];
            ...
            if (!clicked) {
              ...
              console.log('[E2E] Trying to click US note button via JavaScript...');
              const clicked = await page.evaluate(() => {
                const usButton = document.querySelector('[data-value="us"]') ||
              ...
              if (clicked) {
                console.log('[E2E] Successfully clicked US note button via JavaScript');
              }
              await page.waitForTimeout(2000);
            }
          } catch (e) {
      ...
      try {
        ...
        if (await authorityField.isVisible({ timeout: 2000 })) {
          ...
          if (!isEnabled) {
            console.log('[E2E] Authority field is disabled, waiting for form to update...');
            await page.waitForTimeout(2000);
            // Check again
            const isEnabledAfterWait = await authorityField.isEnabled().catch(() => false);
            if (!isEnabledAfterWait) {
              console.log('[E2E] Authority field still disabled after wait');
              // Try clicking the US note button again
              const usNoteButton = page.getByRole('radio', { name: /us note/i }).or(page.locator('input[value="us"]')).first();
          ...
          const authorityValue = await authorityField.inputValue().catch(() => '');
          console.log(`[E2E] Authority field value: "${authorityValue}"`);
          if (!authorityValue || authorityValue.trim() === '') {
            console.log('[E2E] Authority field is empty, filling it manually...');
            // Make sure it's enabled before trying to fill
            const isEnabledNow = await authorityField.isEnabled().catch(() => false);
            if (isEnabledNow) {
              ...
              console.log('[E2E] Authority field is still disabled, cannot fill');
            }
          }
        } else {
      ...
      // Check for form validation errors
      const errorMessages = await page.locator('[class*="error"], [class*="invalid"]').all();
      if (errorMessages.length > 0) {
        console.log(`[E2E] Found ${errorMessages.length} form validation errors`);
      ...
      const obverseImg = await page.locator('img[alt*="obverse"], img[alt*="Obverse"]').first().isVisible().catch(() => false);
      const reverseImg = await page.locator('img[alt*="reverse"], img[alt*="Reverse"]').first().isVisible().catch(() => false);
      console.log(`[E2E] Images present - Obverse: ${obverseImg}, Reverse: ${reverseImg}`);
    ...
    if (!isEnabled) {
      console.log('[E2E] Save button still disabled after all retries. Taking screenshot...');
      ...
      console.log('[E2E] Form field values:', JSON.stringify(formData, null, 2));
    }
    ...
    expect(['United States', 'United States of America']).toContain(banknote.country);
    ...
    if (banknote.watermark !== undefined) {
      expect(banknote.watermark === null || banknote.watermark === '').toBeTruthy();
    }
