...
export const banknoteService = {
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        return records.filter(
          (r) => r.userId === userId && r.isVisibleInCollection
        );
      } else {
        ...
        try {
          ...
          console.warn(
            "Public access not allowed by API rules. Banknotes collection needs to allow public reads for isVisibleInCollection = true"
          );
  ...
  async createBanknote(
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (key === "obverseImage" || key === "reverseImage") {
        ...
        if (value instanceof File) {
          ...
          console.log(
            `[PocketBase] Adding file: ${key} (${value.name}, ${value.size} bytes)`
          );
        }
        ...
        console.log(
          `[PocketBase] Adding field: ${key} = ${value} (type: ${typeof value})`
        );
      }
    });
    ...
    try {
      ...
      console.log("[PocketBase] Created banknote response:", {
        id: created.id,
  ...
  async updateBanknote(
    ...
    const netChange =
      newObverseSize - oldObverseSize + (newReverseSize - oldReverseSize);
    if (netChange !== 0) {
