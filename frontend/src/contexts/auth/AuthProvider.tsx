...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  const [user, setUser] = useState<User | null>(() => {
    // On init, immediately clear any expired token so we never start in a stale-auth state
    ...
    try {
      ...
      if (newUser) {
        try {
          ...
          console.warn(
            "Failed to create subscription on signup:",
            subscriptionError
          );
        }
      }
    ...
    return pb
      ...
      .then(async () => {
        ...
        if (currentUser) {
          ...
          try {
            ...
            console.warn(
              "Failed to create subscription on OAuth signup:",
              subscriptionError
            );
          }
      ...
      .catch((error) => {
        ...
        if (error && typeof error === "object" && "response" in error) {
          const err = error as {
            response?: { data?: unknown; code?: number; message?: string };
          };
