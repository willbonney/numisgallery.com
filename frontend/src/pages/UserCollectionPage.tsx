...
export function UserCollectionPage() {
  ...
  const loadBanknotes = useCallback(async () => {
    ...
    try {
      ...
      if (err?.status === 403 || err?.response?.code === 403) {
        setError(
          "This collection is private or the API rules do not allow public access. Please sign in to view your own collection."
        );
      } else if (err?.status === 404 || err?.response?.code === 404) {
        setError(
          "Collection not found. The user may not exist or have no public banknotes."
        );
      } else {
  ...
  const ownerName =
    ...
    collectionOwner?.email?.split("@")[0] ||
    "Unknown User";
  const collectionTitle = isOwner
    ? "Your Collection"
    : `${ownerName}'s Collection`;

  usePageSeo({
    title: collectionOwner
      ? `${ownerName}'s Banknote Collection | NumisGallery`
      : undefined,
    description: collectionOwner
      ? `Browse ${banknotes.length} public banknote${banknotes.length === 1 ? "" : "s"} in ${ownerName}'s paper money collection on NumisGallery.`
      : undefined,
    path: userId ? `/users/${userId}` : undefined,
  });
