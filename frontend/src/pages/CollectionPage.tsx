import { ActionIcon, Button, Center, Container, FileButton, Group, Loader, Menu, Modal, Progress, Stack, Switch, Text, Title, Tooltip } from '@mantine/core'
import { notifications } from '@mantine/notifications'
import { IconCheck, IconChevronDown, IconFileTypeCsv, IconFileTypePdf, IconPlus, IconShare, IconUpload } from '@tabler/icons-react'
...
export function CollectionPage({ isOwner = true }: CollectionPageProps) {
  const { user } = useAuth()
  const { subscription, getTierLimits, reload: reloadSubscription } = useSubscription()
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (exportType === 'pdf') {
        ...
        await exportCollectionToPDF(displayBanknotes, user?.email, includePurchaseInfo);
        notifications.show({
        ...
        exportCollectionToCSV(displayBanknotes, user?.email, includePurchaseInfo);
        notifications.show({
  ...
  const handleSubmit = async (data: BanknoteFormData & { obverseImage?: File; reverseImage?: File }) => {
    try {
      if (editingBanknote) {
        // Update existing
    ...
    try {
      ...
      for (let i = 0; i < banknotesData.length; i++) {
        try {
          ...
          console.log('[Import] Creating banknote with data:', { country: data.country, countryCode: data.countryCode })
          const created = await banknoteService.createBanknote(data)
          console.log('[Import] Created banknote:', { id: created.id, country: created.country, countryCode: created.countryCode })
          successCount++
        } catch (error) {
    ...
    return (
      <Container size="xl" py="md">
        <BanknoteForm
          ...
          currentFeaturedCount={banknotes.filter(b => b.isFeatured && b.id !== editingBanknote?.id).length}
        />
      </Container>
    )
  }
    ...
    <Container size="xl" py="md">
      <Stack gap="md" mb="lg">
        ...
        <Group justify="space-between" align="center" wrap="wrap" gap="md">
          <Title order={2}>
            {isOwner ? 'Your Collection' : 'Collection'}
          </Title>
        ...
        <Group justify="space-between" align="center" wrap="wrap" gap="md">
          ...
          <Group gap="xl">
            {collectionValue !== '$0' && (
              <div>
                <Text size="xs" c="dimmed" fw={500}>Total collection value</Text>
                <Text size="lg" fw={700}>{collectionValue}</Text>
              </div>
            )}
            <div>
              <Text size="xs" c="dimmed" fw={500}>Banknotes</Text>
              <Text size="lg" fw={700}>{displayBanknotes.length}</Text>
            </div>
            {subscription && (
              <div>
                <Text size="xs" c="dimmed" fw={500}>Storage</Text>
                <Text size="lg" fw={700}>
                  {formatStorageSize(subscription.totalStorageUsed || 0)} / {formatStorageSize(limits.storage.limit)}
                </Text>
              </div>
            )}
          </Group>
          ...
          <Group gap="sm">
            {isPro && (
              ...
              >
                {(props) => (
                  <Tooltip label="Import from Numista (beta)" position="bottom" withArrow>
                    <Button
                      {...props}
                      variant="light"
                  ...
                  {isPro ? (
                    ...
                    <Menu.Item
                      ...
                      onClick={() => {
                        notifications.show({
                          title: 'Pro Feature',
                          message: 'PDF export is available for Pro subscribers. Upgrade to unlock this feature.',
                          color: 'blue',
                        })
                        navigate('/pricing')
                      }}
